...
import { REDIS_IP, REDIS_PORT, REDIS_USERNAME, REDIS_PASSWORD } from "$env/static/private";
...
export async function rateLimit(user, endpoint, timeLimit = 500) {
    const response = await redisClient.set(`rate_limit:${user}:${endpoint}`, "", { PX: timeLimit, NX: true })
