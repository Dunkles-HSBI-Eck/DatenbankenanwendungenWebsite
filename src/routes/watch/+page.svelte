...
<div
	class="fixed bottom-0 p-1 w-full h-32 bg-primary-500 duration-300 opacity-0 hover:opacity-100 transition-opacity hover:pointer-events-auto"
>
	<div class="p-10">
		<Slider
			name="stream"
			id="progressBar"
			bind:value={barPercentage}
			disabled={false}
			readOnly={false}
			...
			thumbSize="size-4"
			thumbClasses="duration-200 opacity-0 hover:opacity-100"
			class="flex right-0"
		/>
	</div>
	<div class="flex flex-nowrap whitespace-nowrap">
		<button class="btn preset-filled-tritiary-500 ml-10" onclick={pauseClick}
			>{#if paused}<IconPlay />{:else}
				<IconPause />{/if}
		</button>
		...
		<div class="flex w-48">
			<button class="btn preset-filled-tritiary-500 ml-10 right-0" onclick={handleMuteButton}
				>{#if volume == 0}
					<IconMuted height="10" />
				{:else}
					<IconSound height="10" />{/if}</button
			>
		</div>

		<button class="btn preset-filled-tritiary-500 ml-10 right-0 flex-none"
			><IconBack height="10" /></button
		>
	</div>
</div>
